...
export const UNSAFE_PRODUCTS_QUESTIONNAIRE = questionsJson as unknown as QuickCheckQuestionFile;
export const UNSAFE_PRODUCTS_STAGES = stagesJson as unknown as QuickCheckStageFile;
export const UNSAFE_PRODUCTS_ACTION_PACKS = actionPacksJson as unknown as QuickCheckActionPackFile;
...
function matchesStage(stage: QuickCheckStage, answers: QuickCheckAnswers): boolean {
  return Object.entries(stage.affectedBy).some(([questionId, optionIds]) => {
...
function resolveMatchingStages(answers: QuickCheckAnswers): QuickCheckStage[] {
  return UNSAFE_PRODUCTS_STAGES.stages.filter((stage) => matchesStage(stage, answers));
}
...
function getActionPackForStage(stageId: string): QuickCheckActionPack | null {
  return UNSAFE_PRODUCTS_ACTION_PACKS.actionPacks.find((pack) => pack.stageId === stageId) ?? null;
}

const OVERLAY_DISPLAY_ORDER: Record<string, { order: number; role: GuideResultSection["role"] }> = {
  S: { order: 1, role: "evidence" },
  E: { order: 2, role: "evidence" },
  H: { order: 3, role: "escalation" },
